...
**智分宝**是一款基于 YOLOv8 轻量级目标检测模型的智能垃圾分类桌面应用，能实时识别 **12 类生活垃圾**并给出分类结果。
...
| 批大小 | 16 | 适配 CPU 训练 |
| 图片尺寸 | 640×640 | 标准输入 |
| 硬件 | Intel i5-13420H (CPU) | 纯 CPU 训练 |
| 数据划分 | 8:2 (训练:验证) | |
...
2. **完整工程化** — 日志系统、配置中心、异常处理、CLI 参数
3. **多线程架构** — 推理与 UI 分离，不阻塞界面操作
4. **数据持久化** — 检测记录自动保存，重启可追溯
5. **人性化交互** — 快捷键、状态指示、实时反馈
